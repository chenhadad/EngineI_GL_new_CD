...
IGL_INLINE void Renderer::resize(GLFWwindow* window,int w, int h)
	...
	void Renderer::Animate() {
		if (scn->anim) {
			if (is_collide(&scn->tree_list[scn->last_picked], &scn->tree_list[!(scn->last_picked)]))
				scn->anim = false;
			else
				scn->data(scn->last_picked).TranslateInSystem(scn->MakeTrans(), scn->direction, true);
	...
	//A_BOX = box of last picked obj
	bool Renderer::is_separate(Eigen::AlignedBox<double, 3> A_BOX, Eigen::AlignedBox<double, 3> B_BOX) {
		Eigen::Vector3d A0 = scn->data(scn->last_picked).GetRotation().cast<double>() * Eigen::Vector3d(1, 0, 0);
		Eigen::Vector3d A1 = scn->data(scn->last_picked).GetRotation().cast<double>() * Eigen::Vector3d(0, 1, 0);
		Eigen::Vector3d A2 = scn->data(scn->last_picked).GetRotation().cast<double>() * Eigen::Vector3d(0, 0, 1);
		Eigen::Matrix3d A;
		A << A0[0], A1[0], A2[0],
			A0[1], A1[1], A2[1],
			A0[2], A1[2], A2[2];
		Eigen::Vector3d B0 = scn->data(!scn->last_picked).GetRotation().cast<double>() * Eigen::Vector3d(1, 0, 0);
		Eigen::Vector3d B1 = scn->data(!scn->last_picked).GetRotation().cast<double>() * Eigen::Vector3d(0, 1, 0);
		Eigen::Vector3d B2 = scn->data(!scn->last_picked).GetRotation().cast<double>() * Eigen::Vector3d(0, 0, 1);
		Eigen::Matrix3d B;
		B << B0[0], B1[0], B2[0],
			B0[1], B1[1], B2[1],
			B0[2], B1[2], B2[2];
		...
		Eigen::Vector4d D4 = scn->data(!scn->last_picked).MakeTrans().cast<double>() * centerB - scn->data(scn->last_picked).MakeTrans().cast<double>() * centerA;
